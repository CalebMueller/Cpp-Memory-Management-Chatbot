...
bool ChatBotApp::OnInit()
{
    ...
    ChatBotFrame *chatBotFrame = new ChatBotFrame(wxT("Caleb's Memory ChatBot"));
...
void ChatBotFrameImagePanel::render(wxDC &dc)
{
    ...
    wxString imgFile = imgBasePath + "Black_Backdrop.jpg";
    wxImage image;
...
{
    wxImage image;
    image.LoadFile(imgBasePath + "panelBackGround.jpg");

    wxSize sz = this->GetSize();
    wxImage imgSmall = image.Rescale(sz.GetWidth(), sz.GetHeight(), wxIMAGE_QUALITY_HIGH);

    _image = wxBitmap(imgSmall);
    dc.DrawBitmap(_image, 0, 0, true);
}
    ...
    : wxPanel(parent, -1, wxPoint(-1, -1), wxSize(-1, -1), wxBORDER_NONE)
{
    ...
    _chatBotImg = new wxStaticBitmap(this, wxID_ANY, (isFromUser ? wxBitmap(imgBasePath + "userIcon.png", wxBITMAP_TYPE_PNG) : *bitmap), wxPoint(-1, -1), wxSize(-1, -1));
